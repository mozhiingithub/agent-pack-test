...
6. **spec 两层先行**：新需求先写 product-spec（`docs/product-specs/active/`，可新建或更新既有），再写 exec-plan（`docs/exec-plans/active/`，须注明来源需求）；exec-plan 经人确认后才动工，合入后归档到 `completed/`。fix 类只写 exec-plan（四要素：现象、根因、方案、影响面），不需要 product-spec。
